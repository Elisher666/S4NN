...
my_array = np.array(matrix.reshape(-1))
print(my_array)
coords = [[] for _ in range(0,257)]

for i, value in enumerate(my_array):
    print(i,'g//',value)
    coords[value].append(i)

df = pd.DataFrame(coords)
df = df.transpose()
# # 使用to_excel()函数将表格数据输出为Excel文件
df.to_excel('dataid.xlsx', index=False)
...
#         if my_array[j]==k:
#             list=np.append(list,j)
#     result_id[k]=list
#     list=()
#
#
# df = pd.DataFrame(result_id,index=range(1,5))
# #
# # # 使用to_excel()函数将表格数据输出为Excel文件
# df.to_excel('dataid.xlsx', index=False)
# print(result_id)
...
# for i in range(256):
#     # 找到所有像素值为i的坐标
#     x,y = np.where(my_array == i)
#     if len(x) == 0 or len(y) == 0:
#         continue
#     # 将坐标添加到对应的行
#     coordinates.loc[i] = np.column_stack((x, y))
